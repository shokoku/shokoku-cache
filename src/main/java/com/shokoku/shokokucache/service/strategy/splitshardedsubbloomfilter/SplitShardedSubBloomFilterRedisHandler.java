...
import com.shokoku.shokokucache.common.distributedlock.DistributedLockProvider;
import com.shokoku.shokokucache.service.strategy.splitshardedbloomfilter.SplitShardedBloomFilter;
import com.shokoku.shokokucache.service.strategy.splitshardedbloomfilter.SplitShardedBloomFilterRedisHandler;
...
public class SplitShardedSubBloomFilterRedisHandler {
  ...
  public static final int MAX_FILTER_COUNT = 2;
  ...
  private void appendSubFilterIfFull(SplitShardedSubBloomFilter splitShardedSubBloomFilter, SplitShardedBloomFilter activated, Long dataCount) {
    ...
    if (!distributedLockProvider.lock(distributedLockKey, Duration.ofSeconds(1))) {
      return;
    }
    try {
      int subFilterCount = findSubFilterCount(splitShardedSubBloomFilter);
      if (subFilterCount >= MAX_FILTER_COUNT) {
